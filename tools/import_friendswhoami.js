 ...
 * gate in front of it:
 *
 *   - a card whose clue count is not a shape the rotation can deal (12, 6 or 3)
...
for (const c of cards) {
  ...
  if (!rounds) {
    faults.push(`${c.id} "${c.name}": ${depth} clues, which is not a shape the rotation deals (12, 6 or 3)`);
    continue;
  }
