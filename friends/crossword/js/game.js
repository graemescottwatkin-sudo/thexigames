...
(function () {
  ...
  function openBoard(target, restore) {
    ...
    try { localStorage.setItem("xifc.mode", kind); } catch (e) {}
  ...
  var ANSWERS_AFTER_KEY = "xifc.answersAfter";
  ...
  var FREE_ARCHIVE_KEY = "xifc.freeArchiveDays";
  ...
  function boardSlot(b) {
    ...
    if (t.kind === "theme") {
      ...
      return "xifc.v04.theme." + k;
    }
    return "xifc.v04.daily." + (t.no || today());
  }
  ...
  var challenge = null;
  var CH_NAME_KEY = "xifc.chName";      // remembered, so a returning player presses Play
  var CH_KEY_KEY = "xifc.entrant";      // one entry each: an id this device keeps
  ...
  var CW_ROUTED = ["daily","check-answer","reveal","verify","finish"];
  function cwApi(p) {
    var q = String(p || "");
    var cut = q.indexOf("?");
    ...
    for (var i = 0; i < CW_ROUTED.length; i++) {
      if (base === "/api/" + CW_ROUTED[i]) return "/api/crossword/crossword_fr/" + CW_ROUTED[i] + tail;
    }
    return q;
  }
  ...
  try {
    var pref = localStorage.getItem("xifc.clubPref");
  ...
  function syncKey(no) { return "xifc.v04.sync.daily." + no; }
  ...
  function pushStateNow() {
    ...
    var no = board.no;
    apiAuth("/api/account/state", { game: "crossword_fr", key: k, state: snap })
      .then(function (r) {
  ...
  function clearRemoteState(no) {
    ...
    apiAuth("/api/account/state", { game: "crossword_fr", key: "daily:" + no, state: null })
  ...
  function slotKey(m) {
    if (m === "practice") return "xifc.v04.practice";
  ...
  function save() {
    ...
    try {
      ...
      localStorage.setItem("xifc.mode", board.kind);
  ...
  var DAILY_PREFIX = "xifc.v04.daily.";
  var THEME_PREFIX = "xifc.v04.theme.";
  ...
  var USED_KEY = "xifc.usedClues.v1";
  var USED_CAP = 2000;
  ...
  function buildPuzzle(restore) {
    ...
    return requestPuzzle(restore).then(function (res) {
      ...
      if (err && err.status === 401 && window.XIChrome && window.XIChrome.archive) {
        ...
        if (window.XIChrome.permalink) window.XIChrome.permalink.clear("crossword_fr");
        showHome();
        return;
      }
      showLoadError(err);
  ...
  function finishBuild(restore) {
    ...
    if (window.XIBar) {
      ...
      window.XIBar.set({ name: "Crossword XI: Friends", no: isDaily ? board.no : null,
  ...
  var RECENT_CAP = 200;
  function loadRecent() {
    try { return JSON.parse(localStorage.getItem("xifc.recent")) || []; }
    catch (e) { return []; }
  }
  function recordRecent(ids) {
    try {
      ...
      localStorage.setItem("xifc.recent", JSON.stringify(list));
    } catch (e) {}
  }
  ...
  try {
    var savedF = JSON.parse(localStorage.getItem("xifc.filter"));
  ...
  function saveFilter() {
    try { localStorage.setItem("xifc.filter", JSON.stringify(filterOn)); } catch (e) {}
  }
  ...
  var bankOn = true;
  try { bankOn = localStorage.getItem("xifc.bank") !== "off"; } catch (e) {}
  ...
  try { skipFilled = localStorage.getItem("xifc.skip") === "on"; } catch (e) {}
  ...
  on("pitchToggle", "click", function () {
    pitchOn = !pitchOn;
    try { localStorage.setItem("xifc.pitch", pitchOn ? "on" : "off"); } catch (e) {}
    applyPitch();
  });
  applyPitch();
  ...
  function playStart(keep) {
    ...
    window.XIPlays.start({
      game: "crossword_fr", mode: board.kind,
    ...
    apiAuth("/api/admin/reset-my-record", {}).then(function () {
      ...
      try {
        ...
        /* Named keys, then everything under the game's prefixes.
           ...
           moment they were keyed per board — "xifc.v04.daily.3" is not
           "xifc.v04.daily", so a reset removed nothing that mattered and the
  ...
  function seasonTestLabel() {
    ...
    if (!b) return;
    var v = null;
    try { v = localStorage.getItem("xifc.seasonStart"); } catch (e) {}
  ...
  on("adminSeasonTest", "click", function () {
    ...
    try {
      if (v) {
        localStorage.removeItem("xifc.seasonStart");
        adminMsg("Season test off. Labels are back to Daily #n.");
      } else {
        var from = FCW.dailyNumber() + 1;
        localStorage.setItem("xifc.seasonStart", String(from));
  ...
  on("skipToggle", "click", function () {
    ...
    try { localStorage.setItem("xifc.skip", skipFilled ? "on" : "off"); } catch (e) {}
  ...
  on("bankToggle", "click", function () {
    bankOn = !bankOn;
    try { localStorage.setItem("xifc.bank", bankOn ? "on" : "off"); } catch (e) {}
  ...
  var WIPE_KEYS = [
    "xifc.results.v1",        // daily and archive results
    "xifc.themeResults.v1",   // themed boards, and what was marked as played
    "xifc.streak",            // legacy: the run, before it was derived
    "xifc.pre",               // legacy: the pre-season record
    "xifc.recent",            // recent answers, so they are not repeated
    "xifc.usedClues.v1",      // clue circulation
    "xifc.v04.daily",         // the three saved games
    "xifc.v04.practice",
    "xifc.v04.theme",
    "xifc.mode",
    "xifc.v04.daily",         // dead: the shared slot before boards had their own
    "xifc.athome",            // whether the menu was left deliberately
    "xifc.seasonStart",       // the owner's season test override
    "xifc.tip.followword",    // the one-time tip
  ...
  var THEME_RESULTS_KEY = "xifc.themeResults.v1";
  ...
  function applyClubChoice(value) {
    if (value === "__random__") {
      clubMode = "random";
      try { localStorage.removeItem("xifc.clubPref"); } catch (e) {}
      ...
      try { localStorage.setItem("xifc.clubPref", club); } catch (e) {}
    }
  ...
  function paintFullTime(res, table, pos) {
    ...
    $("bChecks").textContent = helpPhrase("check", checksUsed, res.checkPenalty);
    $("bCheckPen").textContent = helpMins("check", checksUsed);
    $("bCheckAlls").textContent = helpPhrase("answer", checkAllsUsed, res.checkAllPenalty);
    $("bCheckAllPen").textContent = helpMins("checkAll", checkAllsUsed);
    $("bLetters").textContent = helpPhrase("draw", revealedLetterCount(), res.revealLetterPenalty);
    $("bLetterPen").textContent = helpMins("revealLetter", revealedLetterCount());
    $("bAnswers").textContent = helpPhrase("answer", revealedAnswerCount(), res.revealAnswerPenalty);
    ...
    if (window.XIFullTime && $("nextUpRow")) {
      window.XIFullTime.nextUp($("nextUpRow"), { game: "crossword_fr" });
    }
  ...
  function verifyScore() {
    ...
    api("/api/finish", { token: puzzleToken, playId: playId, letters: letters })
      .then(function (r) {
        ...
        $("bChecks").textContent = helpPhrase("check", r.checks || 0, b.checkPenalty || 0);
        $("bCheckPen").textContent = helpMins("check", r.checks || 0);
        $("bCheckAlls").textContent = helpPhrase("answer", r.checkAlls || 0, b.checkAllPenalty || 0);
        $("bCheckAllPen").textContent = helpMins("checkAll", r.checkAlls || 0);
        $("bLetters").textContent = helpPhrase("draw", r.revealedLetters || 0, b.letterPenalty || 0);
        $("bLetterPen").textContent = helpMins("revealLetter", r.revealedLetters || 0);
        $("bAnswers").textContent = helpPhrase("answer", r.revealedAnswers || 0, b.answerPenalty || 0);
  ...
  /* THE GAME'S OWN ADDRESS, READ OFF THE PAGE RATHER THAN RESTATED.
     ...
     game whether the page came from /friends/crossword/ or from a permalink;
  ...
  function savedFor(which, forBoard) {
    if (which === "practice") {
      try { return JSON.parse(localStorage.getItem("xifc.v04.practice")); } catch (e) { return null; }
    }
  ...
  function renderHome() {
    ...
    if (seasonTile) {
      var on = false;
      try { on = !!localStorage.getItem("xifc.seasonStart"); } catch (e) {}
  ...
  var TIP_KEY = "xifc.tip.followword";
  var tipShown = false;
  ...
  (function () {
    ...
    on("fxFit", "click", function () {
      ...
      applyFxMode();
      try { localStorage.setItem("xifc.fxmode", fxMode); } catch (e) {}
    });
  })();
  ...
  function setHomeVisible(on) {
    ...
    /* Remembered across a refresh, so leaving is not undone by one.
       ...
       to forget. */
    try { localStorage.setItem("xifc.athome", on ? "1" : ""); } catch (e) {}
    if (on) {
      renderRunLine();
      renderPreviousCount();
      try { window.scrollTo(0, 0); } catch (e) {}
  ...
  /* Boot: restore the fit board.kind, then apply the layout.
     ...
     anything runs it. */
  try {
    var fm = localStorage.getItem("xifc.fxmode");
  ...
  (function () {
    ...
    on("tbModePop", "click", function (ev) {
      ...
      /* CLUBS AND THEMES IS A PLACE NOW, NOT A SHEET. It has its own pages —
         /friends/crossword/clubs/, and one per club — so a club can be linked to, and
  ...
  function chooseMode(which, target) {
    if (which === "daily") {
      ...
      if (window.XIChrome && window.XIChrome.permalink) {
        var n = t.no || today();
        if (n === today()) window.XIChrome.permalink.clear("crossword_fr");
        else window.XIChrome.permalink.show("crossword_fr", String(n));
      }
      return;
    }
  ...
  function alreadyPlayedElsewhere(no) {
    ...
    if (!found && no === FCW.dailyNumber() &&
        ...
        window.XIChrome.playedTodayHas("crossword_fr")) {
  ...
  (function boot() {
    ...
    if (perma && /^[0-9]+$/.test(perma)) {
      ...
      if (window.XIChrome && window.XIChrome.permalink) window.XIChrome.permalink.aged("crossword_fr", today() - pn);
      return;
    }
    ...
    var last = null;
    try { last = localStorage.getItem("xifc.mode"); } catch (e) {}
    /* Left deliberately last time: show the menu, whatever is saved. */
    var resumeChoices = 0;
    var atHome = false;
    try { atHome = !!localStorage.getItem("xifc.athome"); } catch (e) {}
    ...
    if (!atHome && !arrived) {
      ...
      /* A themed board has to be found by scanning, for the same reason a daily
         ...
         "xifc.v04.theme.none" and found nothing — which is why a daily resumed
