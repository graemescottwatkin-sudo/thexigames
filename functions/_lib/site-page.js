 ...
 */
/* ONLY THE CROSSWORD HAS A CARD IN THE REPOSITORY, so it is the only entry
   here and every other game falls through to it. Naming a per-game path that
   404s would be worse than a family card: WhatsApp and Reddit both fall back
   to NOTHING on a broken image, so a link that would have had the wrong
   picture gets no picture at all.

   Four cards for the other games are generated by tools/build_og_images.js and
   are sitting in the tree awaiting the owner's look. The day they are
   committed, this map gains four lines and nothing else changes — deploy_check
   refuses a path here with no file behind it, so it cannot gain them early. */
const OG_IMAGE = {
