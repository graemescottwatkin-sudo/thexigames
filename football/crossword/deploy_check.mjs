...
/* The tag has to have MOVED, not merely be consistent.
   ...
   "v001" < "v001b" < "v002". */
const LAST_SHIPPED = "v003q";    // <- what is LIVE; bump after each deploy
...
const LAST_SHIPPED_ASSETS = "d0e61f84f8de6d27";
...
t("the stylesheet's braces and comments balance", (() => {
  /* GUARDED, because an unguarded read here CRASHES the gate rather than
     failing it — and a crash mid-file is not a verdict, it is a stack trace
     where a summary should be. Measured 15 September 2026: moving this game's
     CSS and updating index.html to match threw ENOENT on this line, so every
     check below it never ran, including the one written at the end of this
     file to report exactly that move. The empty string is safe now that that
     block asserts the page names a stylesheet of its own and that every file
     it names exists — the prohibitions below pass on "", which is the whole
     reason those two positive checks had to be added. */
  const css = has("css/style.css") ? read("css/style.css") : "";
  const bare = css.replace(/\/\*[\s\S]*?\*\//g, "");
  return (bare.match(/\{/g) || []).length === (bare.match(/\}/g) || []).length &&
         (css.match(/\/\*/g) || []).length === (css.match(/\*\//g) || []).length;
})(), (() => {
  const bare = (has("css/style.css") ? read("css/style.css") : "").replace(/\/\*[\s\S]*?\*\//g, "");
